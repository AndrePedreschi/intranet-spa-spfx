...
export function InternalNews(): ReactElement {
  ...
  const getData = useCallback(async () => {
    if (context) {
      ...
      const responseSubComments = await getSubCommentsList(context, 1);
      console.log("SubComentários", responseSubComments);
    }
  }, [context]);
  useEffect(() => {
    getData();
    ...
    <div>
      {context && (
        <div>
          ...
          <button onClick={() => postNewSubComment(context, body2)}>
            Postar um sub-comentário
          </button>
        </div>
      )}
    </div>
